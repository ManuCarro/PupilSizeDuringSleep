...
% smusall 2010/4/23  https://uk.mathworks.com/matlabcentral/fileexchange/29534-stdshade
...
amean=fillmissing(amean,'movmean',100,2);
% astd = nanstd(amatrix,[],1); % to get std shading
astd = std(amatrix,[],1,'omitnan')/sqrt(size(amatrix,1)); % to get sem shading
astd =fillmissing(astd,'movmean',100,2);  % replace nan values
if exist('alpha','var')==0 || isempty(alpha) 
    fillOut = fill([F fliplr(F)],[amean+astd fliplr(amean-astd)],acolor,'linestyle','none', 'Marker', 'none');
    acolor='k';
else
    fillOut = fill([F fliplr(F)],[amean+astd fliplr(amean-astd)],acolor, 'FaceAlpha', alpha,'linestyle','none', 'Marker', 'none');
end
if ishold==0
    check=true; else check=false;
end
hold on;
lineOut = plot(F,amean, 'color', acolor,'linewidth',5,'linestyle','-', 'Marker', 'none'); %% change color or linewidth to adjust mean line
if check
    hold off;
end
end
